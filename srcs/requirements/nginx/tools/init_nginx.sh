...
openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/private/nginx-selfsigned.key -out $CERTS_ -subj "/C=BR/L=BR/O=42/OU=student/CN=vipereir.42.fr"
																										# certs é opath
#openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/private/nginx-selfsigned.key -out /etc/ssl/certs/nginx-selfsigned.crt -subj "/C=BR/L=BR/O=42/OU=student/CN=[vipereir.42.fr](http://vipereir.42.fr/)"
